...
void oddElements()
{
	...
	for (int i = 0; i < n; i++)
		cout << arr[i] << " ";
	cout << "\n\nВведите число, на которое уменьшится каждый нечётный элемент: ";
	int a;
	cin >> a;
	cout << "\nИзменённый массив:\n";
	for (int i = 0; i < n; i++) {
	...
	cout << "\n\nКаждый нечётный элемент умножился на случайную цифру: \n";
	for (int i = 0; i < n; i++) {
		if (i % 2 == 1)
	...
	cout << "\n";
	for (int i = 1; i <= 9; ++i)
		cout << "\nКоличество чисел, которые делятся на " << i << ": " << count[i];
	cout << "\n\nНажмите любую кнопку для возвращения в меню";
	_getch();
	system("cls");
}
void menu()
{
	...
	int mainMenu;
	{
		cout << "Что будем делать? \n"
			...
			<< "9 - Изменить нечётные элементы (idz)\n"
			<< "10 - Выход\n\n";
	}
	cin >> mainMenu;
